...
public:
        ...
        buffer (buffer)
    {
        ...
        uint64_t buffer_bits = 64UL - (PF_COUNT_LEADING_ZEROS_64(buffer_capacity)+1);

        uint64_t buffer_number_of_elements = (1UL << buffer_bits)/Page::PF_PAGE_SIZE;
    ...
    ~StringsHashBulkOnDiskImport() {
        ...
        for (uint64_t i = 0; i <= buffer_bitmask; ++i) {
            ...
            if (page_number != UINT64_MAX) {
                buckets_file.seekp(page_number*Page::PF_PAGE_SIZE, buckets_file.beg);
                buckets_file.write(current_page, Page::PF_PAGE_SIZE);
            }
            current_page += Page::PF_PAGE_SIZE;
        }
        buckets_file.close();
        delete[] split_buffer;
    ...
    void create_id(const char* str, uint64_t id, size_t strlen) {
        ...
        while (true) {
            ...
            if (need_split) {
                ...
                uint64_t new_page_number = buckets_file.tellp() / Page::PF_PAGE_SIZE;
                ...
                *new_bucket.local_depth = *bucket.local_depth + 1;
                ...
                buckets_file.write(split_buffer, Page::PF_PAGE_SIZE);
            } else {
                return;
            }
        }
...
private:
    ...
    char* get_page(uint_fast32_t page_number) {
        ...
        auto page = buffer + (Page::PF_PAGE_SIZE*buffer_idx);
        if (old_page_number == page_number) {
            // cache hit
            return page;
        } else if (old_page_number == UINT64_MAX) {
            // load new page from disk
            buckets_file.seekg(page_number*Page::PF_PAGE_SIZE, buckets_file.beg);
            buckets_file.read(page, Page::PF_PAGE_SIZE);

            buffer_page_number[buffer_idx] = page_number;
            return page;
        } else {
            // flush old page to disk
            buckets_file.seekp(old_page_number*Page::PF_PAGE_SIZE, buckets_file.beg);
            buckets_file.write(page, Page::PF_PAGE_SIZE);
            // load new page from disk
            buckets_file.seekg(page_number*Page::PF_PAGE_SIZE, buckets_file.beg);
            buckets_file.read(page, Page::PF_PAGE_SIZE);
