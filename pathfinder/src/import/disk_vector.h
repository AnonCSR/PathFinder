...
public:
    DiskVector(const std::string& filename) :
        filename (filename),
        buffer_size  (Page::PF_PAGE_SIZE*N*sizeof(uint64_t)),
        buffer (reinterpret_cast<char*>(PF_ALIGNED_ALLOC(Page::PF_PAGE_SIZE, buffer_size)))
    {
    ...
    {
        // free append buffer
        PF_ALIGNED_FREE(buffer);
        current_permutation = original_permutation;
        // buffer_size must be divisible by (Page::PF_PAGE_SIZE*N*sizeof(uint64_t)
        auto max_blocks_per_run = max_buffer_size / (Page::PF_PAGE_SIZE*N*sizeof(uint64_t));
        auto new_buffer_size = max_blocks_per_run * (Page::PF_PAGE_SIZE*N*sizeof(uint64_t));
    ...
    void push_back(const std::array<uint64_t, N>&& record) {
        ...
        if (buffer_append_current_pos == Page::PF_PAGE_SIZE) {
    ...
    void begin_tuple_iter() {
        ...
        iter_buffer_offset = Page::PF_PAGE_SIZE - 1;
    }
    ...
    std::array<uint64_t, N>& next_tuple() {
        // max tuples in buffer is Page::PF_PAGE_SIZE
        iter_buffer_offset = (iter_buffer_offset+1) % Page::PF_PAGE_SIZE;
        iter_current_tuple++;

        if (iter_buffer_offset == 0) {
            // leer del disco a buffer
            file.read(buffer, Page::PF_PAGE_SIZE*N*sizeof(uint64_t));
            file.clear();
        }
...
private:
    ...
    void merge_sort(const std::string& base_name, StatsProcessor<N>& stat_processor) {
        ...
        const uint64_t block_size = Page::PF_PAGE_SIZE*N*sizeof(uint64_t);
        ...
        const uint64_t max_tuples_per_block = Page::PF_PAGE_SIZE;
