...
void OnDiskImport::start_import(const std::string& input_filename, const std::string& prefixes_filename) {
    ...
    {   // Initial memory allocation
        external_strings                     = reinterpret_cast<char*>(PF_ALIGNED_ALLOC(Page::PF_PAGE_SIZE, buffer_size));
    ...
    char* pending_string_buffer = reinterpret_cast<char*>(PF_ALIGNED_ALLOC(
        Page::PF_PAGE_SIZE,
    ...
    // we reuse the allocated memory for external strings as a buffer
        ...
        strings_file.close();
        PF_ALIGNED_FREE(pending_string_buffer);
    }
    ...
    PF_ALIGNED_FREE(external_strings);
