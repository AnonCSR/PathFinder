...
BufferManager::BufferManager(uint_fast32_t shared_buffer_pool_size,
    ...
    bytes                     (reinterpret_cast<char*>(PF_ALIGNED_ALLOC(
                                   Page::PF_PAGE_SIZE,
                                   shared_buffer_pool_size * Page::PF_PAGE_SIZE))),
    private_bytes             (reinterpret_cast<char*>(PF_ALIGNED_ALLOC(
                                   Page::PF_PAGE_SIZE,
                                   private_buffer_pool_size * max_threads * Page::PF_PAGE_SIZE))),
...
Page& BufferManager::get_page(FileId file_id, uint_fast32_t page_number) noexcept {
    ...
    if (it == pages.end()) {
        ...
        page.reassign(page_id, &bytes[buffer_available*Page::PF_PAGE_SIZE]);
...
Page& BufferManager::get_tmp_page(TmpFileId tmp_file_id, uint_fast32_t page_number) noexcept {
    ...
    if (it == private_tmp_pages[thread_pos].end()) {
        ...
        page.reassign(page_id,
                      &private_bytes[Page::PF_PAGE_SIZE * (thread_pos*private_buffer_pool_size + buffer_available)]);
