...
class BPTLeafWriter {
public:
    static constexpr auto max_records = (Page::PF_PAGE_SIZE - 2*sizeof(int32_t)) / (sizeof(uint64_t)*N);

    BPTLeafWriter(const std::string& filename) {
        file.open(filename, std::ios::out|std::ios::binary);
        buffer = new char[Page::PF_PAGE_SIZE];
    }
...
public:
    static constexpr auto max_records = (Page::PF_PAGE_SIZE - 2*sizeof(int32_t) )
    ...
    BPTDirWriter(const std::string& filename) {
        ...
        auto root = new char[Page::PF_PAGE_SIZE];
        memset(root, 0, Page::PF_PAGE_SIZE);
        pages.push_back(root);
    }

    ~BPTDirWriter() {
        for (auto page : pages) {
            file.write(page, Page::PF_PAGE_SIZE);
            delete[] page;
        }
        file.close();
    }
                             ...
                             int32_t leaf_page_number)
    {
        ...
        if (split_data.need_split) {
            ...
            // Case 2: non-root split
                ...
                auto new_page = new char[Page::PF_PAGE_SIZE];
                memset(new_page, 0, Page::PF_PAGE_SIZE);
            ...
            // Case 3: root split
                ...
                auto lhs_page = new char[Page::PF_PAGE_SIZE];
                memset(lhs_page, 0, Page::PF_PAGE_SIZE);
                pages.push_back(lhs_page);

                int32_t rhs_page_number = pages.size();
                auto rhs_page = new char[Page::PF_PAGE_SIZE];
                memset(rhs_page, 0, Page::PF_PAGE_SIZE);
                pages.push_back(rhs_page);

                // new_lhs has everything previous
                std::memcpy(lhs_page, pages[dir_page_number], Page::PF_PAGE_SIZE);
