...
void FileManager::flush(Page& page) const {
    ...
    lseek(fd, page.page_id.page_number*Page::PF_PAGE_SIZE, SEEK_SET);
    auto write_res = write(fd, page.get_bytes(), Page::PF_PAGE_SIZE);
    if (write_res == -1) {
...
void FileManager::read_page(PageId page_id, char* bytes) const {
    ...
    lseek(fd, page_id.page_number*Page::PF_PAGE_SIZE, SEEK_SET);
    if (file_size/Page::PF_PAGE_SIZE <= page_id.page_number) {
        // new file page, write zeros
        memset(bytes, 0, Page::PF_PAGE_SIZE);
        auto write_res = ftruncate(fd, Page::PF_PAGE_SIZE*page_id.page_number);
        ...
        auto read_res = read(fd, bytes, Page::PF_PAGE_SIZE);
        if (read_res == -1) {
...
void FileManager::read_big_page(PageId page_id, char* bytes, uint64_t page_size) const {
    ...
    if (file_size/page_size <= page_id.page_number) {
        ...
        auto write_res = ftruncate(fd,page_size*page_id.page_number);
