 ...
 * A page is treated as an array of `PF_PAGE_SIZE` bytes (pointed by `bytes`).
 * For better performance, `PF_PAGE_SIZE` should be multiple of the operating system's page size.
...
public:
    static constexpr size_t PF_PAGE_SIZE = 4096;
    ...
    // get the start memory position of `PF_PAGE_SIZE` allocated bytes
...
private:
    // start memory address of the page, of size `PF_PAGE_SIZE`
    char* bytes;
