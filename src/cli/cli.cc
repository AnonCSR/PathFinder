...
class CLI {
public:
    CLI(Model model, std::chrono::seconds timeout) : model(model), timeout(timeout) { }
    ~CLI();
...
private:
    ...
    std::chrono::milliseconds query_execution_time;
    ...
    std::unique_ptr<std::wfstream> history_fs;
...
void CLI::ReadHistoryFile() {
    ...
    if (home_var == nullptr) {
        AddOutput(L" HOME environment variable not set, not loading command history", LineType::OutputError);
        return;
    }

    auto history_file_path = std::filesystem::path(home_var) / ".pathfinder" / "history";
    if (!std::filesystem::exists(history_file_path)) {
        try {
            std::filesystem::create_directories(history_file_path.parent_path());
        } catch (...) {
            AddOutput(L" Could not create " + history_file_path.parent_path().wstring() + L", not loading command history", LineType::OutputError);
            return;
        }
    } else if (!std::filesystem::is_regular_file(history_file_path)) {
        AddOutput(L' ' + history_file_path.wstring() + L"is not a file, not loading command history", LineType::OutputError);
        return;
    }

    history_fs = std::make_unique<std::wfstream>(history_file_path, std::ios_base::app | std::ios_base::in | std::ios_base::out);

    if (!history_fs->is_open()) {
        AddOutput(L" Could not open " + history_file_path.wstring() + L" in RW mode, not loading command history", LineType::OutputError);
        history_fs = nullptr;
        return;
    }

    history_fs->seekg(std::ios_base::beg);
...
void CLI::AppendHistoryEntryToFile(const std::vector<std::wstring>& history_entry) {
    if (history_fs == nullptr) {
        return;
    }

    history_fs->clear();
    history_fs->seekp(std::ios_base::end);

    if (history_fs->fail()) {
        std::cerr << "Could not write to history file\n";
    }
    ...
    for (auto& line : history_entry) {
        ...
        if (history_fs->fail()) {
            std::cerr << "Error while writing to history file\n";
        }
    }
...
void CLI::HandleKeyDelete() {
    ...
    } else if (cursor_x == static_cast<int>(input_data[cursor_y].size()) && cursor_y + 1 < static_cast<int>(input_data.size())) {
...
void CLI::HandleSend() {
    ...
    auto path_raw   = std::filesystem::temp_directory_path() / ("pf-cli.tsv");
    auto path_table = std::filesystem::temp_directory_path() / ("pf-cli.txt");
    ...
    while (true) {
        ...
        // Handle timeout
        if (qc.thread_info.timeout <= std::chrono::system_clock::now()) {
    ...
    while (wget_wch(input_win, nullptr) != ERR);
    ...
    if (status == ReturnStatus::Interrupted) {
        ...
        if (query_result_count > 0) {
            AddOutput(L" " + std::to_wstring(query_result_count) + L" results in " + std::to_wstring(query_execution_time.count()) + L"ms", LineType::OutputOk);
    ...
    fs_table.clear();
    fs_table.flush();
    fs_table.seekg(std::ios_base::beg);
    if (fs_table.fail()) {
...
void CLI::HandleWordRight() {
    if (cursor_x == static_cast<int>(input_data[cursor_y].size()) && cursor_y + 1 < static_cast<int>(input_data.size())) {
        cursor_y++;
        cursor_x = 0;
    }
...
ReturnStatus CLI::calculate_column_widths(std::istream& input_stream, std::vector<int>& column_widths) {
    ...
    while (true) {
        if (counter++ % 1'000'000 == 0) {
            if (last_time + std::chrono::milliseconds(100) < std::chrono::system_clock::now()) {
                update_process_spinner();
...
void CLI::ProcessSPARQLQuery(std::ostream& os, const std::string& query) {
    try {
        ...
        query_execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - start_time);
...
int CLI::MainLoop() {
    while (true) {
        ...
        {
            std::cerr <<"wget_wch returned and error";
            return EXIT_FAILURE;
        }
    }
}
