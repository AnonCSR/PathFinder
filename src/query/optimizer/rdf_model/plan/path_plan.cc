...
{
    automaton = path.get_rpq_automaton(&RDFObjectId::get_Iri);
    auto inverted_path = path.clone()->invert();
    automaton_inverted = inverted_path->get_rpq_automaton(&RDFObjectId::get_Iri);
}
...
double PathPlan::estimate_output_size() const {
    ...
    for (auto& state_transitions : automaton.from_to_connections) {
        for (auto& transition : state_transitions) {
            ...
            auto it = rdf_model.catalog().predicate2total_count.find(predicate_id);
            if (it != rdf_model.catalog().predicate2total_count.end()) {
                res += it->second * 1.1;
            }
        }
    }
