    ...
    last_probe_key    (Key<N>(last_pk_start))
{
    ...
    data_chunk = new uint64_t[(build_vars.size() + 1) * Page::PF_PAGE_SIZE];
    // Add to directory
    ...
    key_chunk = new uint64_t[N * Page::PF_PAGE_SIZE];
    // Add to directory
...
void Join<N>::build_hash_table() {
    ...
    while (build_rel->next()) {
        ...
        data_chunk_index++;
        if (data_chunk_index == Page::PF_PAGE_SIZE) {
            data_chunk = new uint64_t[data_tuple_size * Page::PF_PAGE_SIZE];
            data_chunks_dir.push_back(data_chunk);
            data_chunk_index = 0;
        }
        ...
        if (key == last_key) {
            ...
            key_chunk_index++;
            if (key_chunk_index == Page::PF_PAGE_SIZE) {
                key_chunk = new uint64_t[N * Page::PF_PAGE_SIZE];
                key_chunks_dir.push_back(key_chunk);
                key_chunk_index = 0;
            }
