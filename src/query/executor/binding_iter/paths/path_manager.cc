...
void PathManager::clear() {
    std::thread::id thread_id = std::this_thread::get_id();
    {
        ...
        // TODO: Currently all executors call path_manager.clear(),
        // however, not all executors call path_manager.begin().
        // Either we have to change how the executors call clear/end,
        // or we have to check if the thread_id is in thread_paths:
        auto it = thread_paths.find(thread_id);
