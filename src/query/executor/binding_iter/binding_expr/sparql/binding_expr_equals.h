...
// The following datatypes have know semantics and special handling in PathFinder.
    ...
    ObjectId eval(const Binding& binding) override {
        ...
        if (lhs_oid.is_numeric() && rhs_oid.is_numeric()) {
            ...
            case Conversions::OPTYPE_INTEGER: {
                ...
                return ObjectId(ObjectId::MASK_BOOL | static_cast<uint64_t>(lhs == rhs));
            }
        ...
        if (lhs_generic_type == ObjectId::MASK_DT && rhs_generic_type == ObjectId::MASK_DT) {
            bool error;
            uint64_t res = DateTime(lhs_oid.id).compare<DateTimeComparisonMode::StrictEquality>(DateTime(rhs_oid.id), &error) == 0;
            if (error) {
                return ObjectId::get_null();
            }
        ...
        if (lhs_subtype == rhs_subtype) {
            ...
            if (lhs_subtype == ObjectId::MASK_STRING_LANG) {
                ...
                return ObjectId(ObjectId::MASK_BOOL | static_cast<uint64_t>(lang_equal && str_equal));
            }
        ...
        if ((lhs_subtype == ObjectId::MASK_STRING_XSD || lhs_subtype == ObjectId::MASK_STRING_SIMPLE) &&
            ...
            uint64_t equals = Conversions::to_lexical_str(lhs_oid) == Conversions::to_lexical_str(rhs_oid);
