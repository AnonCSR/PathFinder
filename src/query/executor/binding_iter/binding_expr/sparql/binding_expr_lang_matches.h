...
public:
    ...
    ObjectId eval(const Binding& binding) override {
        ...
        if (rhs_str == "*") {
            return ObjectId(ObjectId::MASK_BOOL | static_cast<uint64_t>(lhs_size > 0));
        }
        ...
        if (lhs_size < rhs_size) {
            ...
            return ObjectId(ObjectId::MASK_BOOL | static_cast<uint64_t>(lhs_str == rhs_str));
        }

        auto lhs_prefix = lhs_str.substr(0, rhs_str.size());

        if (lhs_str[lhs_prefix.size()] == '-') {
            return ObjectId(ObjectId::MASK_BOOL | static_cast<uint64_t>(lhs_prefix == rhs_str));
        } else {
            return ObjectId(ObjectId::BOOL_FALSE);
        }
    }
